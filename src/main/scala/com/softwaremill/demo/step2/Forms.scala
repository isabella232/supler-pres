...
object Forms {
  val customValidator = custom[Mountain, Int]((v, m) => v != 3129, (v, m) => Message("Cannot be 3129"))

  def mountainForm(deleteAction: Mountain => ActionResult[Mountain]) = form[Mountain](f => List(
    f.field(_.name).label("Name"),
    f.field(_.height).label("Height (meters)").validate(gt(0), le(8848), customValidator),
